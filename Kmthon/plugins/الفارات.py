...
ZelzalVP_cmd = (
    "𓆩 [𝗞𝗺𝘁𝗵𝗼𝗻 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - أوامـر الفـارات](t.me/Kmthon) 𓆪\n\n"
    "**⎉╎قائمـة أوامر تغييـر فـارات الصـور بأمـر واحـد فقـط - لـ أول مـرة ع سـورس كـمـثـون يوزر بـوت 🦾 :** \n\n"
    "⪼ `.اضف صورة الحماية` بالـرد ع صـورة أو ميديـا\n\n"
    "⪼ `.اضف صورة الفحص` بالـرد ع صـورة أو ميديـا\n"
    "⪼ قنـاة كلايـش الفحـص @Km_Vars\n\n"
    ...
    "⪼ `.اضف كليشة الايدي` بالـرد ع الكليشـة مـن القنـاة @Km_Vars \n\n\n"
    ...
    "⪼ قنـاة كلايـش حمايـة الخـاص @Km_Vars\n\n"
    ...
    "\n𓆩 [𝗞𝗺𝘁𝗵𝗼𝗻 𝗩𝗮𝗿𝘀 - قنـاة الفـارات](t.me/Km_Vars) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت كـمـثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم إضافة {} بنجـاح ☑️** \n**⎉╎المضاف اليه :**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت كـمـثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت كـمـثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, variable))
        ...
        if variable not in heroku_var:
        	return await zed.edit("**⎉╎عـذࢪًا عـزيـزي .. انت لـم تقـم بإضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("**⎉╎تم حـذف {} بنجـاح ☑️**\n**⎉╎المتغيـر المحـذوف : ↶**\n `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت كـمـثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def custom_zed(event):
    ...
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        ...
        if variable is None:
            ...
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Kmthon**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("**⎉╎فـار ايموجي/رمز الايدي غيـر موجـود ❌**\n**⎉╎لـ إضـافته بالـرد ع الكليشـة من هنا ( @Kmthonklaesh ) استخـدم الأمـر : ↶**\n `.اضف كليشة الايدي` \n\n**⎉╎قنـاة السـورس : @Kmthon**")
        else:
            await zed.edit("**⎉╎الفـار {} موجـود ☑️**\n**⎉╎قيمـة الفـار : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Kmthon**".format(input_str, variable))
...
async def variable(event):
    ...
    viran = "Asia/Tehran"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**⎉╎جـاري إعـداد المنطقـة الزمنيـة لـ كـمـثـون 🌐...**")
    # All Rights Reserved for "Zed-Thon" "زلـزال الهيبـة"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن" or input_str == "فلسطين":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر المنطقـة الزمنيـة .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الآن إعادة تشغيـل بـوت كـمـثـون يستغـرق الأمر 2-1 دقيقـة ▬▭ ...**".format(input_str))
        else:
    ...
    if malatha.reply_to_msg_id:
        ...
        if input_str in ["الحماية", "الحمايه"]:
            ...
            try:
                ...
                await zed.edit("**⎉╎تم تغييـر صـورة {} .. بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n\n**⎉╎قنـاة السـورس : @Kmthon**".format(input_str, vinfo))
