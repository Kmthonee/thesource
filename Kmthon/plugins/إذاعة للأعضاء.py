...
# =========================================================== #
#                           الملـــفـ كتـــابـــة مـــن الصفــــر - t.me/Kmthon                          #
# =========================================================== #
Warn = "hhh"
KMTHON_BEST_SOURCE = "[ᯓ 𝗞𝗺𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/Kmthon) .\n\n**- جـارِ الاذاعـه خـاص لـ أعضـاء الكـروب 🛗\n- الرجـاء الانتظـار .. لحظـات ⏳**"
KMTHON_PRO_SOURCE = "[ᯓ 𝗞𝗺𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/Kmthon) .\n\n**- جـارِ الاذاعـه لـ قائمـة زاجـل 📜\n- الرجـاء الانتظـار .. لحظـات ⏳**"
NAYZAK_PRO_DEV = "[ᯓ 𝗞𝗺𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/Kmthon) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎قائمـة الاذاعـه فـارغـه ؟! ❌**\n**⎉╎قم باضافـة يوزرات عبـر الامر**\n`.اضفـ زاجل` **بالـرد ع عدة يوزرات تفـصل بينهم مسـافـات**"
# =========================================================== #
#                                      نــايــزك الهيبـــه - T.me/iNayzak                                #
# =========================================================== #
...
async def malath(event):
    ...
    zelzal = await event.edit(KMTHON_BEST_SOURCE, link_preview=False)
    total = 0
    success = 0
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if zilzal.text:
            ...
            try:
                ...
                return
    ZELZAL_BEST_DEV = f"[ᯓ 𝗞𝗺𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/Kmthon) .\n\n**⎉╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**⎉╎عـدد {success} عضـو**"
...
async def malath(event):
    ...
    if gvarstatus("ZAGL_Zed") is None:
        return await event.edit(NAYZAK_PRO_DEV, link_preview=False)
    zelzal = gvarstatus("ZAGL_Zed")
    users = zelzal.split(" ")
    zzz = await event.edit(KMTHON_PRO_SOURCE, link_preview=False)
    total = 0
    success = 0
    user_entity = None
    for user in users:
        ...
        if zilzal.text:
            ...
            try:
                ...
                zzz.edit(f"خطـأ فـي إرسـال الرسـالة إلــى {user_entity.id}: {str(e)}")
    ZELZAL_BEST_DEV = f"[ᯓ 𝗞𝗺𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/Kmthon) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎تمت الاذاعـه .. بنجـاح  ✅**\n**⎉╎عـدد {success} أشخـاص**"
