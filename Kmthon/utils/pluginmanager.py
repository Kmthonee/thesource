...
from Kmthon import CMD_HELP, LOAD_PLUG
...
LOGS = logging.getLogger("سورس كمثون")
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"Kmthon/plugins/{shortname}.py")
        checkplugins(path)
        name = "Kmthon.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"Kmthon/plugins/{shortname}.py")
            name = f"Kmthon.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"Kmthon.plugins.{shortname}"] = mod
        LOGS.info(f"تـم تثبيت ملـف {shortname}")
