...
from Kmthon import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def mybot(): #Code by T.me/zzzzl1l
    ...
    if bot_name.startswith("مسـاعـد"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "Kmthon")
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"Kmthon/{folder}/*.py"
        plugin_path = f"Kmthon/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photozed = await zedub.upload_file(file="zedthon/malath/Kmthon.jpg")
        _, groupid = await create_supergroup(
            "مجموعـة السجـل كمثون", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    await load_plugins(folder="Kmthon", extfolder=cfolder)
