...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as Kmthon:
            print("Generating a string session for •Kmthon•")
            try:
                Kmthon.send_message(
                    "me",
                    f"**- هـذا هو كـود سيشـن تيليثـون الخـاص بـك ☑️**:\n\n`{Kmthon.session.save()}`\n\n**- لا تشـارك هـذا الكـود مع اي احـد ⚠️؟!**\n**- فقـط قم بنسخـه ولصقـه في مربعـات التنصيب 🛃**\n\n**- سـورس تيبـثون : @Kmthon**",
                )
                print(
        ...
        print(er)
        print("If you think It as a Bug, Report to @iNayzak\n\n")
...
def pyro_session():
    ...
    with Client(
        name="Kmthon", api_id=API_ID, api_hash=API_HASH, in_memory=True
    ) as pyro:
